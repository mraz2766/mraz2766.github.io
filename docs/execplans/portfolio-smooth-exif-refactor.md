...
继续将站点从“摄影栏目叙事”压缩为“极简摄影作品集”。文字只保留导航、分类、数量、按钮和必要状态；PC 端强调大图、留白和网格秩序，H5 端强调轻量筛选、双列浏览和不拥挤的首屏。

### 计划修改

- 首页只保留照片封面、站名、极短副标题和单一入口。
- 首页下方保留精选照片与分类入口，分类不再带描述。
- 作品页和专题页删除长说明，只保留 `Works`、数量、分类筛选和工具按钮。
- 复查截图后删除作品页/专题页顶部重复大标题，只保留图库工具栏作为标题与操作入口。
- 图库工具栏从说明型 header 改成操作型 toolbar。
- 大图查看器无 EXIF 时显示 `No EXIF`，不再展示长句。
- 分类默认使用 `All / Pets / Toys`，保留路由不变。

### 进度

- [x] 已压缩站点文案与分类文案。
- [x] 已重构首页为极简封面、精选和分类入口。
- [x] 已简化作品页、专题页与图库工具栏。
- [x] 已调整 PC/H5 响应式样式。
- [x] 已完成 lint/build 和截图检查。

### 决策记录

- 继续保留当前照片生成流水线、Lightbox 和 EXIF 结构。
- 不新增依赖，不调整路由。
- Markdown 文档继续使用中文。

### 结果

- 已完成 `npm run gen-gallery`、`npm run lint`、`npm run build`。
- 已检查 PC/H5 首页与作品页截图；首页为照片封面，作品页只保留工具栏、分类、照片网格和分页。
- Lightbox 文案已压缩为标题、分类、序号、尺寸、EXIF 或 `No EXIF`。
